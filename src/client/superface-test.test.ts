...
import { generate } from '../hash-generator';
import { MatchErrorLength } from '../matcher/errors';
import {
  canUpdateTraffic,
...
import { SuperfaceTest } from '.';
import { InputVariables } from './superface-test.interfaces';
...
jest.mock('../recording', () => ({
  ...jest.requireActual('../recording'),
  startRecording: jest.fn(),
  endAndProcessRecording: jest.fn(),
  processAndLoadRecordings: jest.fn(),
...
const prepareRecordingsConfig = (options?: RecordingsOptions) => ({
  ...
  recordingsType: options?.recordingsType ?? recordingsConfig.type,
...
describe('SuperfaceTest', () => {
  ...
  afterEach(() => {
    ...
    mocked(processAndLoadRecordings).mockReset();
    mocked(endAndProcessRecording).mockReset();
  ...
  describe('run', () => {
    describe('when recording', () => {
      it('writes and restores recordings', async () => {
        ...
        const endAndProcessRecordingSpy = mocked(endAndProcessRecording);
        ...
        expect(endAndProcessRecordingSpy).toHaveBeenCalledTimes(1);
        expect(endAndProcessRecordingSpy).toHaveBeenCalledWith(
      ...
      it('saves report if new recording contains some changes', async () => {
        ...
        mocked(prepareSuperface).mockResolvedValue(mockSuperface());
        mocked(matchWildCard).mockReturnValueOnce(true);
        mocked(endAndProcessRecording).mockResolvedValue({
    ...
    describe('when loading recordings', () => {
      it('loads fixture if it exists, but contains no recordings', async () => {
        const processAndLoadRecordingsSpy = mocked(processAndLoadRecordings);
        ...
        expect(processAndLoadRecordingsSpy).toHaveBeenCalledTimes(1);
        expect(processAndLoadRecordingsSpy).toHaveBeenCalledWith(
